...
- **Superseded in part**: see Postscript, 2026-08-02 — the series names were wrong
- **Date**: 2026-06-25
...
## Postscript 2026-08-02: the series names were never those of a live vLLM

This ADR chose `vllm:prefix_cache_hits` and `vllm:prefix_cache_queries`
as the series to read. Those names are correct in vLLM's source, where
`v1/metrics/loggers.py` registers the counters under exactly that
spelling. They are not the names a vLLM endpoint emits.

vLLM builds those counters with `prometheus_client`, which appends
`_total` on exposition unless the name already carries it. The token
counters this ADR reads alongside them — `vllm:prompt_tokens_total`,
`vllm:generation_tokens_total` — were written with the suffix and
therefore always matched. The two prefix-cache series were written as
registered, and matched nothing on any real vLLM endpoint. Verified
against `prometheus_client` 0.26.0: of the six series this crate reads,
four resolve and these two do not.

The consequence is that no KV-cache hit rate in this repository was ever
measured against vLLM. Every test and every kind rehearsal read
`llm-d-inference-sim`, which spells the token counters as vLLM exposes
them and the prefix-cache counters as vLLM registers them. The simulator
answered, so the schema looked correct. The A10 evidence of 2026-07-21
carries `cache_hits_delta: 0` and `cache_queries_delta: 0` on all five
steps for this reason, not because the cache went unused.

The failure was not silent by design. `parse_kvcache` requires the
denominator and returned `Parse` on every scrape; `scrape_during`
swallows per-tick errors so that a transient endpoint failure does not
abort a run, and a systematic failure is indistinguishable from a slow
endpoint under that policy. Downstream, deltas over a degenerate
timeline read as `Some(0)`, and zero is a plausible hit rate. Three
correct local decisions composed into a wrong figure.

Two things this postscript does not claim. The published agentic-KV
result of 2026-07-30 is unaffected: that experiment scrapes with its own
Python reader using a prefix match, which resolves the suffixed name,
and its figures stand. And `vllm-coldstart-operator` had already found
this live on A10 + vLLM 0.23.0 and fixed it in its own reporter with a
suffixed-then-bare lookup — the lesson existed in a sibling repository
and did not cross the boundary to this one.

Fixed in `cd0ece6`. The schema now names the exposed series, the test
fixture is generated by `prometheus_client` rather than transcribed, and
a test pins that the simulator is not a vLLM endpoint for KV. The
capability is correct from that commit; it remains unmeasured on real
hardware until a GPU run produces a rate.

Divergence worth stating: the operator falls back to the bare name after
trying the suffixed one, so it reads both vLLM and the simulator. This
crate does not, because ADR-014 D1 forbids inferring a vocabulary from
the body. The two choices are deliberate and different.
